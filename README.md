...
Для взаимодействия микросервисов и веб-клиента используется REST API поверх HTTP.

**Обоснование:**
- Для сценариев управления устройствами (включить/выключить отопление, свет, открыть/закрыть ворота) пользователю нужен немедленный ответ о результате выполнения команды
- REST хорошо подходит для стандартных CRUD-операций (регистрация устройств, получение списка устройств, чтение истории телеметрии)
- Для интеграции с внешними системами и фронтендом широко доступны инструменты и библиотеки, а спецификацию REST API удобно описывать с помощью OpenAPI/Swagger

### 2. Документация API

[OpenAPI документация основных сервисов](api/openapi.yaml)
